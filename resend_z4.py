#!/usr/bin/env python3
"""Resend 4 low/zero-CTR campaigns with v2 optimized copies. 9 packs each."""
...
# CTR=0%: S1b,S2 → new direction. CTR<3%: G3,G5 → optimized
TASKS = [
    ("smart", "S1b-baka-nandyan-na-v2",
     "{$phone[4]} teka lang, may nakita ako kanina na para sa yo, pakitingnan mo nga ito ${shortUrl}", 1),
    ("smart", "S2-profile-update-v2",
     "{$phone[4]} hoy kamusta na? ang tagal mo na di nagche-check ng account mo ah ${shortUrl}", 2),
    ("globe", "G3-napansin-ko-lang-v2",
     "{$phone[10]} uy may nakita akong update sa side mo, parang ok to para sa yo ${shortUrl}", 2),
    ("globe", "G5-baka-makalimutan-v2",
     "{$phone[10]} di ba sabi ko sayo may something ako para sa yo, nandito na oh ${shortUrl}", 3),
]
...
def main():
    print("=" * 50)
    print("重发 4个低/零CTR (v2优化文案, 9包/活动)")
    print("=" * 50)
    ...
    for ti, (carrier, suffix, sms, ch_idx) in enumerate(TASKS):
        ...
        tmpl_id = get_template_id(db, carrier, suffix.replace("-v2", "-v1"))

        print(f"\n{'='*50}")
        print(f"[{ti+1}/4] {carrier}/{suffix}")
