...
GLOBE_CH = [YO_G1, YO_G4, YO_G5, YO_G11, YO_G12, YO_G13, YO_G14]   # 7通道
SMART_CH = [YO_S2, YO_S6, YO_S7, YO_S8, YO_S9, YO_S10]             # 6通道

# CTR=0% auto channel switch: blocked channels with recovery timestamp
BLOCKED_CH = {}  # {channel_id: unblock_after_timestamp}

# 永久禁用的通道: GG全家网全部 + 十五Globe + 三Smart
PERMA_BLOCKED = {GG_AAA, GG_BBB, YO_G15, YO_S3}
...
PACK_PAGES = [25, 26, 27]  # IRON: kt09固定第25-27页，铁律不可修改 (2026-05-03 Willy)
PACK_PAGE_SIZE = 20

def fetch_packs(carrier, db, count=30):
    """IRON RULE: kt09固定page=25-27, q=凯特ai发送. 0包刷新最多3次, 冷却[5,10,15]=30s总量."""
    used = set(r[0] for r in db.execute("SELECT pack_id FROM pack_used").fetchall())
    packs = []
    Q = urllib.parse.quote("凯特ai发送：银河v数据4月")
